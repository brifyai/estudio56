...
export interface VideoGenerationOptions {
  ...
  model?: 'veo-3.1-fast-001' | 'veo-2.0-generate-preview' | 'veo-2.0-flash-generate-preview';
  duration?: '6s' | '8s';
}
...
export const generateVideo = async (
  ...
  console.log('🎬 [VertexVideo] Modelo:', options.model || 'veo-3.1-fast-001');
  ...
  try {
    const response = await fetch('/.netlify/functions/generate-video', {
      ...
      body: JSON.stringify({
        ...
        model: options.model || 'veo-3.1-fast-001',
