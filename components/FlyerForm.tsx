...
export const FlyerForm: React.FC<FlyerFormProps> = ({
  ...
  // NEW: Usar props externas para trigger de análisis de Canva
  const [internalCanvaAnalyzeTrigger, setInternalCanvaAnalyzeTrigger] = useState(0);
  const canvaAnalyzeTrigger = canvaAnalyzeTriggerProp !== undefined ? canvaAnalyzeTriggerProp : internalCanvaAnalyzeTrigger;
  const setCanvaAnalyzeTrigger = setCanvaAnalyzeTriggerProp || setInternalCanvaAnalyzeTrigger;
  ...
  return (
    <>
      ...
      <div className="space-y-4">
         ...
         {/* MODO CANVA: Input de URL y botón analizar */}
         {creationMode === 'canva' && (
           <div className="space-y-4">
             <div className="bg-gradient-to-br from-blue-500/10 to-purple-500/10 border border-blue-500/20 rounded-xl p-4">
               ...
               <div className="space-y-3">
                 <input
                   ...
                   className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2.5 text-sm text-white placeholder-white/30 focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/30"
                 />
                 <button
                   onClick={() => {
                     if (canvasUrlInput.trim()) {
                       setCanvasAnalyzeTrigger(prev => prev + 1);
                     }
                   }}
                   disabled={!canvasUrlInput.trim()}
                   className="w-full bg-blue-500 hover:bg-blue-600 disabled:bg-white/10 disabled:text-white/30 text-white px-4 py-2.5 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2"
                 >
