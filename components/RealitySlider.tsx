  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2 mb-3">
        ...
        {/* Botón Comparar - Habilitado cuando hay 1+ variación */}
        <button
          onClick={onOpenComparator}
          disabled={disabled || Object.keys(cachedVariations).length < 1}
          className={`flex-1 py-2 px-3 rounded-xl text-xs font-medium transition-all flex items-center justify-center gap-2
            ${Object.keys(cachedVariations).length < 1
              ? 'bg-white/5 border border-white/10 text-white/30 cursor-not-allowed'
              : 'bg-purple-500/20 border border-purple-500/50 text-purple-300 hover:bg-purple-500/30 hover:border-purple-500/70'
            }
          `}
          title={Object.keys(cachedVariations).length < 1 ? 'Genera variaciones primero' : 'Comparar realismos'}
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
