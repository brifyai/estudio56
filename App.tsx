...
progressAlert.updateProgress(60, 'Renderizando...');
  ...
  const handleRealityChange = async (newLevel: number) => {
    ...
    try {
      ...
      const result = await generateFlyerImage(
        technicalPrompt, // ✅ Solo descripción técnica de calidad, NO contenido
        styleKey,
        aspectRatio,
        'draft',
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-[100dvh] w-full bg-[#030303] text-white font-sans selection:bg-blue-500/30 relative overflow-hidden">
       ...
       <aside className={`w-full lg:w-[400px] flex-shrink-0 flex flex-col z-20 h-auto p-1 lg:p-2 pr-0 lg:pr-0 ${showRealityComparator ? 'lg:min-h-screen' : 'lg:h-screen'}`}>
      ...
      {showRealityComparator && (
        ...
        `}>
          ...
          <div className="glass-panel rounded-xl lg:rounded-[2rem] w-full max-w-[320px] max-h-[80vh] flex flex-col shadow-2xl overflow-hidden relative z-10 lg:max-h-none lg:h-full">
            {/* Botón cerrar - Flotante en esquina superior derecha, 15px arriba del borde */}
            <button
              onClick={() => setShowRealityComparator(false)}
              className="absolute -top-4 right-2 z-20 w-8 h-8 flex items-center justify-center bg-red-500/80 hover:bg-red-500 rounded-full transition-colors cursor-pointer shadow-lg"
